...
class Portfolio:
    ...
    def load_transactions(self, portfolio_id):
        return self.session.query(Transaction).filter(Transaction.portfolio_id == portfolio_id).all()

    def load_positions(self, portfolio_id):
        return self.session.query(Position).filter(Position.portfolio_id == portfolio_id).all()

    def load_assets(self, portfolio_id):
        return self.session.query(Asset).filter(Asset.portfolio_id == portfolio_id).all()
